...
// === 2) 圖片上傳（自動呼叫 AI → 寫入 detections → 比對黑名單）===
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['act']??'')==='upload_img' && isset($_FILES['image'])) {
  $img = $_FILES['image'];

  // 2-1 基本檔案檢查
  if ($img['error'] !== UPLOAD_ERR_OK) { $flash_err = '上傳失敗（錯誤代碼 '.$img['error'].'）'; }
  else {
    ...
    if (!in_array($ext, ['jpg','jpeg','png','webp'])) {
      ...
      // 2-3 先把影像寫進 detections（用 prepared+send_long_data，避免 BLOB 損壞）
      $stmt = $db->prepare("INSERT INTO detections (image_mime, image_blob) VALUES (?, ?)");
      $null = NULL;
      $stmt->bind_param("sb", $mime, $null);
      $stmt->send_long_data(1, $bin);
      if (!$stmt->execute()) {
        $flash_err = '寫入資料庫失敗：'.$stmt->error;
        $stmt->close();
      } else {
        $detId = $stmt->insert_id;
        $stmt->close();

        // 2-4 呼叫 Python 偵測 API
        $ch = curl_init(DETECT_API_URL);
        $cfile = new CURLFile($img['tmp_name'], $mime, $img['name']);
        ...
        $resp = curl_exec($ch);
        if ($resp === false) {
          $flash_err = '偵測服務無回應：'.curl_error($ch);
          curl_close($ch);
        } else {
          curl_close($ch);
          $data  = json_decode($resp, true);
          $plate = isset($data['plate_text']) ? strtoupper(trim($data['plate_text'])) : null;

          // 2-5 更新車牌文字
          if ($plate) {
            $stmt = $db->prepare("UPDATE detections SET plate_text=? WHERE id=?");
            $stmt->bind_param("si", $plate, $detId);
            $stmt->execute();
            $stmt->close();
          }
          ...
          $hit = 0; $bid = null;
          if ($plate) {
            $stmt = $db->prepare("SELECT id FROM blacklist WHERE plate_no=? AND is_active=1 LIMIT 1");
            $stmt->bind_param("s", $plate);
            $stmt->execute();
            $stmt->bind_result($foundId);
            if ($stmt->fetch()) { $hit = 1; $bid = (int)$foundId; }
            $stmt->close();
          }
          $stmt = $db->prepare("UPDATE detections SET is_blacklisted=?, matched_blacklist_id=? WHERE id=?");
          // matched_blacklist_id 可為 NULL
          $stmt->bind_param("isi", $hit, $bid, $detId);
          $stmt->execute();
          $stmt->close();

          // 2-7 顯示結果
          if ($plate) {
            $flash_ok = '偵測成功：<b>'.e($plate).'</b>'.($hit ? '（命中黑名單）' : '（未命中）');
          } else {
            $flash_ok = '圖片已上傳，但未偵測到車牌';
          }
        }
      }
    }
